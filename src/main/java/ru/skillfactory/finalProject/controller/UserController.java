...
public class UserController {
    ...
    @Transactional
    @GetMapping("/transferMoney/{sender_id}/{recipient_id}")
    ResponseEntity transferMoneyById(
            @PathVariable long sender_id,
            @PathVariable long recipient_id,
            @RequestParam(value = "money") BigDecimal money) {
        return ResponseEntity.status(HttpStatus.OK).body(userService.transferMoney(sender_id, recipient_id, money));
    }
}